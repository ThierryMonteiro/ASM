.data
	...
	#string para salvar valores em hexadecimal
	hex_buffer: .asciiz "0x12345678"
	
	#string para salvar valores em decimal
	dec_buffer: .asciiz "-1234267890"
...
main_loop:
	...
	la $t6, endereco_instrucao #carrega o valor do endere�o em $t6
	
	jal converte_hex_para_string #transforma o numero do endereco para string e escreve no arquivo
	
	jal escreve_espaco #escreve o carater espa�o no arquivo txt
	
	jal encontra_num_instrucao #carrega o valor da instru��o em $t6 e escreve no arquivo
	
	jal escreve_espaco #escreve o carater espa�o no arquivo 
...
escreve_rt:
	...
	#achar o campo rt
	#carrega o valor da intru��o em $t2
...
converte_hex_para_string:
	...
	move $t1, $a0             # Copiar el número de word a $t1
	li $t2, 16                # Cargar 16 en $t2 (base hexadecimal)
	la $t3, hex_buffer        # Cargar la dirección del buffer en $t3
	addi $t3, $t3, 9  
    
	# Convertir el word a cadena hexadecimal (de derecha a izquierda)
	li $t4, 8                 # Inicializar el contador del bucle (8 dígitos hexadecimales)

convert_loop1:
	divu $t5, $t1, $t2        # Dividir el número por 16
	mfhi $t1                  # Obtener el residuo (dígito hexadecimal)
...
less_than_10:
	addi $t1, $t1, 48         # Convertir el dígito a su valor ASCII

continue:
	sb $t1, 0($t3)            # Almacenar el dígito en el buffer (de derecha a izquierda)
	move $t1, $t5
	subi $t4, $t4, 1          # Decrementar el contador del bucle
	subi $t3, $t3, 1
	bnez $t4, convert_loop1    # Bucle hasta que hayamos convertido todos los dígitos
...
converte_dec_para_string:

	lw $a0, 0($t6)  	  # converte o valor que se encontra em $t6
	move $t1, $a0             # Copiar el número de word a $t1
	li $t2, 10                # Cargar 10 en $t2 (base decimal)
	la $t3, dec_buffer        # Cargar la dirección del buffer en $t3
	addi $t3, $t3, 9  
    
	# Convertir el word a cadena hexadecimal (de derecha a izquierda)
	li $t4, 0                 # Inicializar el contador del bucle (10 dígitos hexadecimales)

	srl $t7, $t1, 15
	beq $t7, $zero, numero_positivo
	
	lw $t7, mask
	addu $t1, $t1, $t7
	subi $t1, $t1, 1
	nor $t1, $t1, $zero
	addi $t7, $zero, 45 	  # menos
	j convert_loop2

numero_positivo:
	addi $t7, $zero, 0 	  # menos
	
	
convert_loop2:	
	divu $t5, $t1, $t2        # Dividir el número por 10
	mfhi $t1                  # Obtener el residuo (dígito decimal)
    
	addi $t1, $t1, 48         # Convertir el dígito a su valor ASCII

	sb $t1, 0($t3)            # Almacenar el dígito en el buffer (de derecha a izquierda)
	move $t1, $t5
	addi $t4, $t4, 1          # Inrementar el contador del bucle
	subi $t3, $t3, 1
	bnez $t5, convert_loop2    # Bucle hasta que hayamos convertido todos los dígitos
    	
    	
    	beq $t7, $zero, sem_sinal
    	sb $t7, 0($t3)
    	addi $t4, $t4, 1          # Inrementar el contador del bucle
	j escrever_dec
	#subi $t3, $t3, 1 
	#sb $zero, 11($t3)        # Terminador nulo para finalizar la cadena
sem_sinal:	    
	addi $t3, $t3, 1
	#escrever no arquivo txt
escrever_dec:
